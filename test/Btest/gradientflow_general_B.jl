...
function gradientflow_test_4D_Bfields(NX, NY, NZ, NT, NC)
    Dim = 4
    Nwing = 0
    ...
    flux = zeros(Int, 6)
    flux = rand(0:NC-1, 6)
    B = Initialize_Bfields(NC, flux, Nwing, NX, NY, NZ, NT)

    temps = Temporalfields(U[1]; num=13)
    ...
    g = Gradientflow_general_Bfields(U, B, listloops, listvalues, eps=0.01)
...
@testset "4D" begin
    ...
    @testset "NC=2" begin
        ...
        @time plaq_t = gradientflow_test_4D_Bfields(NX, NY, NZ, NT, NC)
