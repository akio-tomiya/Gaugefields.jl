...
function _g2_projection_site_matrix(U, ix, iy, iz, it)
...
@testset "G2 projection to momentum" begin
    ...
    expected = project_to_g2_coefficients(_g2_projection_site_matrix(raw, 1, 1, 1, 1))
...
@testset "G2 exptU" begin
    ...
    @test norm(_g2_projection_site_matrix(expU, 1, 1, 1, 1) - expected) < 1.0e-12
    @test is_g2_link(_g2_projection_site_matrix(expU, 1, 1, 1, 1); atol = 1.0e-10)
            ...
            for iy in 1:2
                for ix in 1:2
                    if (ix, iy, iz, it) != (1, 1, 1, 1)
                        @test _g2_projection_site_matrix(expU, ix, iy, iz, it) == Matrix{ComplexF64}(I, 7, 7)
                    end
                end
            end
        end
...
@testset "G2 update times link" begin
    ...
    @test is_g2_link(_g2_projection_site_matrix(W, 1, 1, 1, 1); atol = 1.0e-10)
    @test norm(_g2_projection_site_matrix(W, 1, 1, 1, 1) - _g2_projection_site_matrix(expU, 1, 1, 1, 1)) < 1.0e-12
end
