...
function HMC_test_4D(NX, NY, NZ, NT, NC, β)
    ...
    U = Initialize_Gaugefields(NC, Nwing, NX, NY, NZ, NT, condition="hot", randomnumber="Reproducible", mpi=true, PEs=PEs, mpiinit=false)
    #"Reproducible"
    println(typeof(U))
...
function HMC_test_2D(NX, NT, NC, β)
    Dim = 2
    Nwing = 0
    ...
    U = Initialize_Gaugefields(NC, Nwing, NX, NT, condition="hot", randomnumber="Reproducible", mpi=true, PEs=PEs2, mpiinit=false)
...
@testset "3D" begin
    ...
    @testset "NC=1" begin
        ...
        @time plaq_t, ratio = HMC_test_3D(NX, NY, NT, NC, β)
        @test ratio > 0.5
...
@testset "2D" begin
    ...
    @testset "NC=1" begin
        ...
        #val =0.6414596466929057
        val = 0.9768786716327604
        @time plaq_t, ratio = HMC_test_2D(NX, NT, NC, β)
        @test ratio > 0.5
        #@test abs(plaq_t-val)/abs(val) < eps
    end
