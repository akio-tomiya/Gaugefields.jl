...
function gradientflow_test_4D(NX, NY, NZ, NT, NC)
    ...
    temp2cpu = tempscpu[2] #similar(U[1])
    #temp1 = similar(U[1])
    #temp2 = similar(U[1])
    ...
    for i = 1:10
        println("JACC plaq poly ")
        @time plaq_t = calculate_Plaquette(U, temp1, temp2) * factor
        println("0 plaq_t = $plaq_t")
        @time poly = calculate_Polyakov_loop(U, temp1, temp2)
        println("0 polyakov loop = $(real(poly)) $(imag(poly))")

        println("cpu plaq poly")
        @time plaq_t = calculate_Plaquette(Ucpu, temp1cpu, temp2cpu) * factor
        println("0 plaq_t = $plaq_t")
        @time poly = calculate_Polyakov_loop(Ucpu, temp1cpu, temp2cpu)
        println("0 polyakov loop = $(real(poly)) $(imag(poly))")
    end
