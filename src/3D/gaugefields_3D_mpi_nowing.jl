...
#=
module Gaugefields_3D_mpi_module
    using LinearAlgebra
...
"""
`Gaugefields_3D_nowing_mpi{NC} <: Gaugefields_3D{NC}`

MPI version of SU(N) Gauge fields in four dimensional lattice. 
"""
struct Gaugefields_3D_nowing_mpi{NC} <: Gaugefields_3D{NC}
    ...
    function Gaugefields_3D_nowing_mpi(
        NC::T,
        NX::T,
        NY::T,
        NT::T,
...
function get_myrank(U::T) where {T<:Gaugefields_3D_nowing_mpi}
    return U.myrank
end

function get_myrank(U::Array{T,1}) where {T<:Gaugefields_3D_nowing_mpi}
    return U[1].myrank
end

function get_nprocs(U::T) where {T<:Gaugefields_3D_nowing_mpi}
    return U.nprocs
end

function get_nprocs(U::Array{T,1}) where {T<:Gaugefields_3D_nowing_mpi}
    return U[1].nprocs
end


function barrier(x::T) where {T<:Gaugefields_3D_nowing_mpi}
    #println("ba")
    MPI.Barrier(comm)
end

function Base.setindex!(x::Gaugefields_3D_nowing_mpi, v, i1, i2, i3, i6)
    error(
...
function Base.getindex(x::Gaugefields_3D_nowing_mpi, i1, i2, i3, i6)
    error(
...
function Base.setindex!(
    ...
    i3,
    i6,
) where {T<:Gaugefields_3D_nowing_mpi} #U'
...
@inline function getvalue(x::Gaugefields_3D_nowing_mpi, i1, i2, i3, i6)
    @inbounds return x.U[i1, i2, i3, i6]
end

@inline function setvalue!(x::Gaugefields_3D_nowing_mpi, v, i1, i2, i3, i6)
...
function identityGaugefields_3D_nowing_mpi(
    NC,
    NX,
    NT,
    PEs;
...
function randomGaugefields_3D_nowing_mpi(
    NC,
    NX,
    NT,
    PEs;
...
function clear_U!(U::Gaugefields_3D_nowing_mpi{NC}) where {NC}
    for it = 1:U.PN[2]
        #for iz=1:U.PN[3]
        #for iy=1:U.PN[2]
        for ix = 1:U.PN[1]
...
function clear_U!(U::Gaugefields_3D_nowing_mpi{NC}, iseven::Bool) where {NC}
    for it = 1:U.PN[2]
        #for iz=1:U.PN[3]
        #for iy=1:U.PN[2]
        for ix = 1:U.PN[1]
...
function clear_U!(
    U::Gaugefields_3D_nowing_mpi{NC},
    filternumber::N,
    filterindex::N,
) where {NC,N<:Integer}
    for it = 1:U.PN[2]
...
function add_U!(c::Gaugefields_3D_nowing_mpi{NC}, a::T1) where {NC,T1<:Abstractfields}
    for it = 1:c.PN[2]
        #for iz=1:c.PN[3]
        #for iy=1:c.PN[2]
        for ix = 1:c.PN[1]
...
function add_U!(
    c::Gaugefields_3D_nowing_mpi{NC},
    a::T1,
    iseven::Bool,
...
function substitute_U!(
    a::Array{T1,1},
    b::Array{T2,1},
) where {T1<:Gaugefields_3D_nowing_mpi,T2<:Gaugefields_3D_nowing_mpi}
    for μ = 1:2
...
function map_U!(
    U::Gaugefields_3D_nowing_mpi{NC},
    f!::Function,
    V::Gaugefields_3D_nowing_mpi{NC},
    iseven::Bool,
) where {NC}
...
function map_U_sequential!(U::Gaugefields_3D_nowing_mpi{NC}, f!::Function, Uin) where {NC}
    error("The function map_U_sequential! can not be used with MPI")
end



struct Shifted_Gaugefields_3D_mpi_nowing{NC} <: Shifted_Gaugefields{NC,2}
    parent::Gaugefields_3D_nowing_mpi{NC}
    #parent::T
    shift::NTuple{2,Int8}
    NX::Int64
    #NY::Int64
    ...
    function Shifted_Gaugefields_3D_mpi_nowing(
        U::Gaugefields_3D_nowing_mpi{NC},
        shift,
    ) where {NC}
        #shifted_U!(U,shift)
...
function shifted_U_improved_zeroshift!(U::Gaugefields_3D_nowing_mpi{NC}) where {NC}
    for it = 1:U.PN[2]
        #for iz=1:U.PN[3]
        #for iy=1:U.PN[2]
        for ix = 1:U.PN[1]
...
function update_sent_data!(
    ...
    myrank_send = get_myrank_3D(myrank_xt_send, PEs)
...
function shifted_U_improved_xshift!(U::Gaugefields_3D_nowing_mpi{NC}, shift) where {NC}
    tP = 0
...
function shifted_U_improved_tshift!(U::Gaugefields_3D_nowing_mpi{NC}, shift) where {NC}
    xP = 0
...
function mpi_updates_U_1data!(U::Gaugefields_3D_nowing_mpi{NC}, send_ranks) where {NC}
...
function get_myrank_3D(myrank_xyzt, PEs)
    @inbounds return myrank_xyzt[2] * PEs[1] + myrank_xyzt[1]
end

function mpi_updates_U_moredata!(U::Gaugefields_3D_nowing_mpi{NC}, send_ranks) where {NC}
...
function mpi_updates_U!(U::Gaugefields_3D_nowing_mpi{NC}, send_ranks) where {NC}
    if length(send_ranks) != 0
...
function shifted_U_improved!(U::Gaugefields_3D_nowing_mpi{NC}, shift) where {NC}
    if shift == (0, 0)
...
function shifted_U!(U::Gaugefields_3D_nowing_mpi{NC}, shift) where {NC}
    PEs = U.PEs
    PN = U.PN
    myrank = U.myrank
    myrank_xt = U.myrank_xt
...
@inline function getvalue(
    U::Shifted_Gaugefields_3D_mpi_nowing{NC},
    i1,
    i2,
    i3,
    i6,
...
function shift_U(U::Gaugefields_3D_nowing_mpi{NC}, ν::T) where {T<:Integer,NC}
    if ν == 1
        shift = (1, 0)
    elseif ν == 2
        shift = (0, 1)
    ...
    return Shifted_Gaugefields_3D_mpi_nowing(U, shift)
end

function shift_U(
    U::TU,
    shift::NTuple{Dim,T},
) where {Dim,T<:Integer,TU<:Gaugefields_3D_nowing_mpi}
    return Shifted_Gaugefields_3D_mpi_nowing(U, shift)
end



function normalize_U!(U::Gaugefields_3D_nowing_mpi{NC}) where {NC}
...
function Base.similar(U::T) where {T<:Gaugefields_3D_nowing_mpi}
    Uout = Gaugefields_3D_nowing_mpi(
        U.NC,
        U.NX,
        U.NT,
        U.PEs,
...
function Base.similar(U::Array{T,1}) where {T<:Gaugefields_3D_nowing_mpi}
...
function LinearAlgebra.tr(a::Gaugefields_3D_nowing_mpi{NC}) where {NC}
    NX = a.NX
    #NY=a.NY
    #NZ=a.NZ
    NT = a.NT
...
) where {NC,Dim,T<:Gaugefields_3D_nowing_mpi}
    Uold = temp1
    Unew = temp2
...
function partial_tr(a::Gaugefields_3D_nowing_mpi{NC}, μ) where {NC}
...
function set_wing_U!(u::Array{Gaugefields_3D_nowing_mpi{NC},1}) where {NC}
    return
    for μ = 1:2
        set_wing_U!(u[μ])
    end
...
function set_wing_U!(u::Gaugefields_3D_nowing_mpi{NC}) where {NC}
    return
end
