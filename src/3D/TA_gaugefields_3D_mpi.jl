struct TA_Gaugefields_3D_mpi{NC,NumofBasis} <: TA_Gaugefields_3D{NC}
    a::Array{Float64,3}
    NX::Int64
    #NY::Int64
    #NZ::Int64
    ...
    function TA_Gaugefields_3D_mpi(u::Gaugefields_3D{NC}) where {NC}
        ...
        return new{NC,NumofBasis}(
            zeros(Float64, NumofBasis, u.PN[1], u.PN[2]),
            u.NX,
            u.NT,
            u.NC,
            NumofBasis,
...
function Base.setindex!(x::T, v, i...) where {T<:TA_Gaugefields_3D_mpi}
    @inbounds x.a[i...] = v
end

function Base.getindex(x::T, i...) where {T<:TA_Gaugefields_3D_mpi}
    @inbounds return x.a[i...]
end


function barrier(x::TA_Gaugefields_3D_mpi)
    MPI.Barrier(comm)
end

function add_U!(
    c::TA_Gaugefields_3D_mpi{NC,NumofBasis},
    α::N,
    a::TA_Gaugefields_3D_mpi{NC,NumofBasis},
...
function clear_U!(U::TA_Gaugefields_3D_mpi{NC,NumofBasis}) where {NC,NumofBasis}
...
function gauss_distribution!(
    p::TA_Gaugefields_3D_mpi{NC,NumofBasis};
    σ=1.0,
) where {NC,NumofBasis}
    d = Normal(0.0, σ)
    #NumofBasis = Uμ.NumofBasis
...
function Base.:*(
    x::TA_Gaugefields_3D_mpi{NC,NumofBasis},
    y::TA_Gaugefields_3D_mpi{NC,NumofBasis},
) where {NC,NumofBasis}
    #NumofBasis = Uμ.NumofBasis
    s = 0.0
...
function Traceless_antihermitian_add!(
    c::TA_Gaugefields_3D_mpi{3,NumofBasis},
    factor,
    vin::Union{Gaugefields_3D_nowing_mpi{3},Gaugefields_3D_nowing_mpi{3}},
) where {NumofBasis}
...
function Traceless_antihermitian_add!(
    c::TA_Gaugefields_3D_mpi{2,NumofBasis},
    factor,
    vin::Union{Gaugefields_3D_nowing_mpi{2},Gaugefields_3D_nowing_mpi{2}},
) where {NumofBasis}
    #error("Traceless_antihermitian! is not implemented in type $(typeof(vout)) ")
    fac12 = 1 / 2
    NX = vin.NX
...
function Traceless_antihermitian!(
    c::TA_Gaugefields_3D_mpi{NC,NumofBasis},
    vin::Union{Gaugefields_3D_nowing_mpi{NC},Gaugefields_3D_nowing_mpi{NC}},
) where {NC,NumofBasis}
    @assert NC != 3 && NC != 2
    #NC = vout.NC
    fac1N = 1 / NC
    ...
    for it = 1:vin.PN[2]
        ...
        @simd for ix = 1:vin.PN[1]
            ...
            @simd for k2 = k1+1:NC
                vv =
                    0.5 *
                    (getvalue(vin, k1, k2, ix, it) - conj(getvalue(vin, k2, k1, ix, it)))
                #vout[k1,k2,ix, it] = vv
        ...
        @simd for ix = 1:vin.PN[1]
            ...
            for k1 = 1:NC
                @simd for k2 = k1+1:NC
                    vv =
                        0.5 * (
                            getvalue(vin, k1, k2, ix, it) -
                            conj(getvalue(vin, k2, k1, ix, it))
                        )
                    #vout[k1,k2,ix, it] = vv
                    #vout[k2,k1,ix, it] = -conj(vv)
                    matrix[k1, k2] = vv
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_mpi{NC,NumofBasis},
    temps::Array{T,1},
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_mpi{3,NumofBasis},
    temps::Array{T,1},
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_mpi{2,NumofBasis},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_3D,NumofBasis} #uout = exp(t*u)     
    NT = u.NT
    #NZ = u.NZ
