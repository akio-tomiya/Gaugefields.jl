...
function Base.setindex!(x::T, v, i...) where {T<:TA_Gaugefields_3D_serial}
    x.a[i...] = v
end

function Base.getindex(x::T, i...) where {T<:TA_Gaugefields_3D_serial}
    return x.a[i...]
end
...
function substitute_U!(
    Uμ::TA_Gaugefields_3D_serial{NC,NumofBasis},
    pwork,
) where {NC,NumofBasis}
    NT = Uμ.NT
    #NZ = Uμ.NZ
...
function Base.:*(
    x::TA_Gaugefields_3D_serial{NC,NumofBasis},
    y::TA_Gaugefields_3D_serial{NC,NumofBasis},
) where {NC,NumofBasis}
    NT = x.NT
    #NZ = x.NZ
    #NY = x.NY
...
function add_U!(
    ...
    α::N,
    a::TA_Gaugefields_3D_serial{NC,NumofBasis},
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_serial{1,NumofBasis},
    temps::Array{T,1},
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_serial{NC,NumofBasis},
    temps::Array{T,1},
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_serial{3,NumofBasis},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_3D,NumofBasis} #uout = exp(t*u)     
    ww = temps[1]
    w = temps[2]
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_3D_serial{2,NumofBasis},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_3D,NumofBasis} #uout = exp(t*u)     
    NT = u.NT
    #NZ = u.NZ
...
function gauss_distribution!(
    p::TA_Gaugefields_3D_serial{NC,NumofBasis};
    σ=1.0,
) where {NC,NumofBasis}
    d = Normal(0.0, σ)
    NT = p.NT
