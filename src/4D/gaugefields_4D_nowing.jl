...
using Random
    ...
    struct Gaugefields_4D_nowing{NC} <: Gaugefields_4D{NC}
        ...
        function Gaugefields_4D_nowing(NC::T,NX::T,NY::T,NZ::T,NT::T;verbose_level = 2) where T<: Integer
            NV = NX*NY*NZ*NT
            NDW = 0
    ...
    function substitute_U!(a::Array{T1,1},b::Array{T2,1}) where {T1 <: Gaugefields_4D_nowing,T2 <: Gaugefields_4D_nowing}
        for μ=1:4
            substitute_U!(a[μ],b[μ])
        end
    end

    function substitute_U!(a::Array{T1,1},b::Array{T2,1},iseven) where {T1 <: Gaugefields_4D_nowing,T2 <: Gaugefields_4D_nowing}
        for μ=1:4
    ...
    function Base.similar(U::T) where T <: Gaugefields_4D_nowing
        Uout = Gaugefields_4D_nowing(U.NC,U.NX,U.NY,U.NZ,U.NT,verbose_level = U.verbose_print.level)
    ...
    function substitute_U!(a::Gaugefields_4D_nowing{NC},b::T2,iseven::Bool) where {NC, T2 <: Abstractfields}
        NT = a.NT
        NZ = a.NZ
        NY = a.NY
        NX = a.NX
    ...
    function randomGaugefields_4D_nowing(NC,NX,NY,NZ,NT;verbose_level = 2,randomnumber="Random")
    ...
    function RandomGauges_4D(NC,NX,NY,NZ,NT;verbose_level=2,randomnumber="Random")
        return randomGaugefields_4D_nowing(NC,NX,NY,NZ,NT,verbose_level = verbose_level,randomnumber=randomnumber)
    end
            ...
            for iz=1:NZ
                ...
                for iy=1:NY
                    ...
                    for ix=1:NX
                        ...
                        for k2=1:NC                            
                            for k1=1:NC
                                U.Ushifted[k1,k2,ix,iy,iz,it] = U[k1,k2,ix_shifted,iy_shifted,iz_shifted,it_shifted]
                            end
                        end
                    end
                end
    ...
    function shift_U(U::TU,shift::NTuple{Dim,T}) where {Dim,T <: Integer,TU <: Gaugefields_4D_nowing}
        return Shifted_Gaugefields_4D_nowing(U,shift)
    end


    @inline function Base.getindex(U::Shifted_Gaugefields_4D_nowing{NC},i1,i2,i3,i4,i5,i6) where NC
    ...
    function Base.getindex(u::Staggered_Gaugefields{T,μ},i1,i2,i3,i4,i5,i6) where {T <: Gaugefields_4D_nowing,μ}
        NT = u.parent.NT
        NZ = u.parent.NZ
        NY = u.parent.NY
        NX = u.parent.NX
    ...
    function Base.getindex(u::Staggered_Gaugefields{Shifted_Gaugefields_4D_nowing{3},μ},i1,i2,i3,i4,i5,i6)  where {μ}
    ...
    Λ = TA(M)
    """
    function construct_Λmatrix_forSTOUT!(Λ,δ_current::Gaugefields_4D_nowing{NC},Q,u::Gaugefields_4D_nowing{NC}) where NC
        NT = u.NT
        NY = u.NY
        NZ = u.NZ
        NX = u.NX
    ...
    function exptU!(uout::T,t::N,u::Gaugefields_4D_nowing{NC},temps::Array{T,1}) where {N <: Number, T <: Gaugefields_4D_nowing, NC} #uout = exp(t*u)
    ...
    function exptU!(uout::T,t::N,v::Gaugefields_4D_nowing{2},temps::Array{T,1}) where {N <: Number, T <: Gaugefields_4D_nowing} #uout = exp(t*u)
        NT = v.NT
        NZ = v.NZ
        NY = v.NY
        NX = v.NX
    ...
    # #=
    function exptU!(uout::T,t::N,v::Gaugefields_4D_nowing{3},temps::Array{T,1}) where {N <: Number, T <: Gaugefields_4D_nowing} #uout = exp(t*u)
        ww = temps[1]
        w = temps[2]
...
-----------------------------------------------------c
    ...
    function Traceless_antihermitian!(vout::Gaugefields_4D_nowing{2},vin::Gaugefields_4D_nowing{2})
        #error("Traceless_antihermitian! is not implemented in type $(typeof(vout)) ")
        fac13 = 1/3
        NX = vin.NX
        NY = vin.NY
    ...
    function Traceless_antihermitian!(vout::Gaugefields_4D_nowing{NC},vin::Gaugefields_4D_nowing{NC}) where NC
        #NC = vout.NC
        fac1N = 1/NC
        nv = vin.NV
    ...
    function LinearAlgebra.tr(a::Gaugefields_4D_nowing{NC},b::Gaugefields_4D_nowing{NC}) where NC
        NX=a.NX
        NY=a.NY
        NZ=a.NZ
        NT=a.NT
    ...
    function add_U!(c::Gaugefields_4D_nowing{NC},a::T1,iseven::Bool) where {NC,T1 <: Abstractfields}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function add_U!(c::Gaugefields_4D_nowing{NC},α::N,a::T1) where {NC,T1 <: Abstractfields, N<:Number}
        #@inbounds for i=1:length(c.U)
        #    c.U[i] += α*a.U[i]
        #end
        #return 
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{NC},a::T1,b::T2) where {NC,T1 <: Abstractfields,T2 <: Abstractfields}
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{NC},a::T1,b::T2,iseven::Bool) where {NC,T1 <: Abstractfields,T2 <: Abstractfields}
        @assert NC != 2 && NC != 3 "This function is for NC != 2,3"
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{NC},a::T1,b::T2) where {NC,T1 <: Number,T2 <: Abstractfields}
        @assert NC != 2 && NC != 3 "This function is for NC != 2,3"
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{NC},a::T1,b::T2,α::Ta,β::Tb) where {NC,T1 <: Abstractfields,T2 <: Abstractfields,Ta <: Number, Tb <: Number}
        @assert NC != 2 && NC != 3 "This function is for NC != 2,3"
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
...
=#
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{2},a::T1,b::T2,iseven::Bool) where {T1 <: Abstractfields,T2 <: Abstractfields}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{2},a::T1,b::T2) where {T1 <: Abstractfields,T2 <: Abstractfields}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{2},a::T1,b::T2,α::Ta,β::Tb) where {NC,T1 <: Abstractfields,T2 <: Abstractfields,Ta <: Number, Tb <: Number}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{3},a::T1,b::T2) where {T1 <: Abstractfields,T2 <: Abstractfields}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{3},a::T1,b::T2,iseven::Bool) where {T1 <: Abstractfields,T2 <: Abstractfields}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function LinearAlgebra.mul!(c::Gaugefields_4D_nowing{3},a::T1,b::T2,α::Ta,β::Tb) where {NC,T1 <: Abstractfields,T2 <: Abstractfields,Ta <: Number, Tb <: Number}
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
        NX = c.NX
    ...
    function mul_skiplastindex!(c::Gaugefields_4D_nowing{NC},a::T1,b::T2) where {NC,T1 <: Abstractfields,T2 <: Abstractfields}
        #@assert NC != 2 && NC != 3 "This function is for NC != 2,3"
        NT = c.NT
        NZ = c.NZ
        NY = c.NY
