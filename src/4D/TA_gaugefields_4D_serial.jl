...
function substitute_U!(
    Uμ::TA_Gaugefields_4D_serial{NC,NumofBasis},
    pwork,
) where {NC,NumofBasis}
    NT = Uμ.NT
    NZ = Uμ.NZ
...
function Base.:*(
    x::TA_Gaugefields_4D_serial{NC,NumofBasis},
    y::TA_Gaugefields_4D_serial{NC,NumofBasis},
) where {NC,NumofBasis}
    NT = x.NT
    NZ = x.NZ
    NY = x.NY
...
function add_U!(
    c::TA_Gaugefields_4D_serial{NC,NumofBasis},
    α::N,
    a::TA_Gaugefields_4D_serial{NC,NumofBasis},
...
function Traceless_antihermitian_add!(
    c::TA_Gaugefields_4D_serial{3,NumofBasis},
    factor,
    vin::Union{Gaugefields_4D_wing{3},Gaugefields_4D_nowing{3}},
) where {NumofBasis}
...
function Traceless_antihermitian_add!(
    c::TA_Gaugefields_4D_serial{2,NumofBasis},
    factor,
    vin::Union{Gaugefields_4D_wing{2},Gaugefields_4D_nowing{2}},
) where {NumofBasis}
    #error("Traceless_antihermitian! is not implemented in type $(typeof(vout)) ")
    fac12 = 1 / 2
    NX = vin.NX
...
function Traceless_antihermitian!(
    c::TA_Gaugefields_4D_serial{NC,NumofBasis},
    vin::Union{Gaugefields_4D_wing{NC},Gaugefields_4D_nowing{NC}},
) where {NC,NumofBasis}
    @assert NC != 3 && NC != 2
    #NC = vout.NC
    fac1N = 1 / NC
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_4D_serial{NC,NumofBasis},
    temps::Array{T,1},
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_4D_serial{3,NumofBasis},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_4D,NumofBasis} #uout = exp(t*u)     
    ww = temps[1]
    w = temps[2]
...
function exptU!(
    uout::T,
    t::N,
    u::TA_Gaugefields_4D_serial{2,NumofBasis},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_4D,NumofBasis} #uout = exp(t*u)     
    NT = u.NT
    NZ = u.NZ
