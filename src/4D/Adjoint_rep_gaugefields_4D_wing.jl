...
function construct_Adjoint_rep_Gaugefields_4D_wing(Uin::Gaugefields_4D{NC};verbose_level=2) where NC
    return Adjoint_rep_Gaugefields_4D_wing(Uin,verbose_level=verbose_level)
end

function Adjoint_rep_Gaugefields_4D_wing(Uin::Gaugefields_4D{NC};verbose_level=2)  where NC
    NX = Uin.NX
    NY = Uin.NY
    NZ = Uin.NZ
    NT = Uin.NT
...
struct Shifted_Adjoint_rep_Gaugefields_4D{NC,NumofBasis} <: Shifted_Gaugefields{NC,4} 
    ...
    function Shifted_Adjoint_rep_Gaugefields_4D(U::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},shift) where {NC,NumofBasis}
...
function shift_U(U::TU,shift::NTuple{Dim,T}) where {Dim,T <: Integer,TU <: Adjoint_rep_Gaugefields_4D}
    return Shifted_Adjoint_rep_Gaugefields_4D(U,shift)
end

function Base.setindex!(U::Shifted_Adjoint_rep_Gaugefields_4D,v,i1,i2,i3,i4,i5,i6)  where NC
    error("type $(typeof(U)) has no setindex method. This type is read only.")
end

@inline function Base.getindex(U::Shifted_Adjoint_rep_Gaugefields_4D{NC,NumofBasis},i1,i2,i3,i4,i5,i6) where {NC,NumofBasis}
    if U.outside != true   
...
function substitute_U!(a::Vector{T1},b::Vector{T2}) where {NC, NumofBasis,T1 <: Adjoint_rep_Gaugefields_4D_wing,T2 <: Abstractfields}
...
function LinearAlgebra.mul!(c::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},a::T1,b::T2,iseven::Bool) where {NC,T1 <: Abstractfields,T2 <: Abstractfields,NumofBasis}
...
function LinearAlgebra.tr(a::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},b::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis}) where {NC,NumofBasis}
    NX=a.NX
    NY=a.NY
    NZ=a.NZ
    NT=a.NT
...
function LinearAlgebra.tr(a::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis}) where {NC,NumofBasis}
    NX=a.NX
    NY=a.NY
    NZ=a.NZ
    NT=a.NT
...
function partial_tr(a::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},μ) where {NC,NumofBasis}
    NX=a.NX
    NY=a.NY
    NZ=a.NZ
    NT=a.NT
...
function normalize_U!(u::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis}) where {NC,NumofBasis}
    NX = u.NX
    NY = u.NY
    NZ = u.NZ
    NT = u.NT
...
function add_U!(c::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},a::T1) where {NC,T1 <: Abstractfields,NumofBasis}
...
function add_U!(c::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},a::T1,iseven::Bool) where {NC,T1 <: Abstractfields,NumofBasis}
    NT = c.NT
    NZ = c.NZ
    NY = c.NY
    NX = c.NX
...
function add_U!(c::Adjoint_rep_Gaugefields_4D_wing{NC,NumofBasis},α::N,a::T1) where {NC,T1 <: Abstractfields, N<:Number,NumofBasis}
    #@inbounds for i=1:length(c.U)
    #    c.U[i] += α*a.U[i]
    #end
    #return 
