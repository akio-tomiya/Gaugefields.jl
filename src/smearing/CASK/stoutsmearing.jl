function STOUT_Layer(U::Vector{<:AbstractGaugefields{NC,Dim}}, attentionmatrix::WeightMatrix_layer) where {NC,Dim}
    T = eltype(U)
    numg = 5 + Dim - 1 + 2 + 4
    ...
    return STOUT_Layer{T,Dim,Tρ}(attentionmatrix, dataset, Uinα, Uinβ, eQs, Cs, Qs, temps, dSdCs, islocal, false, hasdSdCs, dSdρ)
end

function forward!(s::STOUT_Layer{T,Dim}, Uout, attentionmatrix::WeightMatrix_layer, Uinα, Uinβ) where {T,Dim} #Uout = exp(Q(Uin,ρs))*Uinα
...
function backward_dSdUαUβρ_add!(s::STOUT_Layer{T,Dim,Tρ}, dSdUα, dSdUβ::Vector{<:AbstractGaugefields{NC,Dim}}, dSdρ::Array{N,7}, dSdUout) where {NC,T,Dim,Tρ<:WeightMatrix_layer,N<:Number}
    @assert Dim == 4 "Dim = $Dim is not supported yet. Use Dim = 4"
    temps, itemps = get_temp(s.temps, 4)
    temp1, itemp1 = get_temp(s.temps)#temps[1]
    dSdQ, idSdQ = get_temp(s.temps)# = temps[2]
    dSdΩ, idSdΩ = get_temp(s.temps)# = temps[3]
    dSdUdag, idSdUdag = get_temp(s.temps)# = temps[4]
    ...
    for μ = 1:Dim
        ...
        #display(Qμ[:, :, 1, 1, 1, 1])
        #println("μ = $μ d")
        calc_dSdQ!(dSdQ, dSdUout[μ], Qμ, s.Uin[μ], temp1) #(U^{alpha} dS/dQ ) star dexp(Q)/dQ
        #println("Uin[$μ]")
        ...
        #Cμi = temps[4]
        Cμi, iCμi = get_temp(s.temps)#temps[4]
        for ν = 1:Dim
            if μ == ν
                continue
            end
            for si = 1:maxS
                ...
                evaluate_gaugelinks!(Cμi, loops, s.Uinβ, temps)
    ...
    for μ = 1:Dim
        #for μ = 1:Dim
        calc_dSdUν_fromdSCμ_add!(dSdUβ[μ], s.dataset, s.dSdCs, s.ρs, s.Uinβ, s.temps, μ)
...
function backward_dSdρ_add!(s::STOUT_Layer{T,Dim,Tρ}, dSdρ::Array{N,7}, dSdUout) where {T,Dim,Tρ<:WeightMatrix_layer,N<:Real}
    @assert Dim == 4 "Dim = $Dim is not supported yet. Use Dim = 4"
...
function calc_C!(C, μ, attentionmatrix::WeightMatrix_layer, dataset::Vector{STOUT_dataset{Dim}}, Uin, temps_g) where {Dim}
    temp1, it1 = get_temp(temps_g)
    #temp2, it2 = get_temp(temps_g)
    temp3, it3 = get_temp(temps_g)
    #temp4, it4 = get_temp(temps_g)
    temps, its = get_temp(temps_g, 4)
    ...
    for ν = 1:Dim
        ...
        for s = 1:maxS
            ...
            evaluate_gaugelinks!(temp3, loops, Uin, temps)
            #println("temp3")
...
function LdCdU_i_add!(LdCdU, L, A, B, attentionmatrix::WeightMatrix_layer, temps_g, μ, ν, s) #dCdU = ρ sum_i A_i otimes B_i , dCdagdU = ρ sum_i Abar_i otimes Bbar_i
    BL, iBL = get_temp(temps_g)
    BLA, iBLA = get_temp(temps_g)
    ...
    aBLA, iaBLA = get_temp(temps_g)#[1]
    mul!(aBLA, view(attentionmatrix.data, μ, ν, s, :, :, :, :), BLA)
    add_U!(LdCdU, aBLA)
    unused!(temps_g, iBLA)
    unused!(temps_g, iaBLA)
    #add_U!(LdCdU, ρ, BLA)
end
...
function calc_dSdUν_fromdSCμ_add!(dSdUμ, dataset::Vector{STOUT_dataset{Dim}}, dSdCs, attentionmatrix::WeightMatrix_layer, Us, temps_g, μ) where {Dim}  #use pullback for C(U): dS/dCμ' star dCμ'/dUμ
    temp1, it1 = get_temp(temps_g)#[1]
    temp2, it2 = get_temp(temps_g)#temps_g[2]
    temp3, it3 = get_temp(temps_g)#temps_g[3]
    temp4, it4 = get_temp(temps_g)#temps_g[4]
    temps, itemps = get_temp(temps_g, 4)
        ...
        for ν = 1:Dim
            ...
            for s = 1:maxS
                ...
                for j = 1:numdCμddUμ
                    ...
                    #show(leftlinks)
                    if numleft != 0
                        evaluate_gaugelinks!(A, leftlinks, Us, temps)# [temp1, temp2])
                    ...
                    B = temp4
                    if numright != 0
                        evaluate_gaugelinks!(B, rightlinks, Us, temps)#[temp2, temp3])
                        mul!(temp2, B, temp1)
                    else
...
function calc_dSdUν_fromdSCμ_add!(dSdUν, dataset::Vector{STOUT_dataset{Dim}}, dSdCμ, attentionmatrix::WeightMatrix_layer, Us, temps_g, μ, ν) where {Dim}  #use pullback for C(U): dS/dCμ star dCμ/dUν
    temp1, itemp1 = get_temp(temps_g)#[1]
    temp2, itemp2 = get_temp(temps_g)#temps_g[2]
    temp3, itemp3 = get_temp(temps_g)#temps_g[3]
    temp4, itemp4 = get_temp(temps_g)#temps_g[4]
    ...
    unused!(temps_g, itemp1)
    unused!(temps_g, itemp2)
    unused!(temps_g, itemp3)
    unused!(temps_g, itemp4)
