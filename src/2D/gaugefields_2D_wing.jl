...
function substitute_U!(
    a::Array{T1,1},
    b::Array{T2,1},
) where {T1<:Gaugefields_2D_wing,T2<:Gaugefields_2D_wing}
    for μ = 1:2
...
function Base.similar(U::T) where {T<:Gaugefields_2D_wing}
    Uout =
        Gaugefields_2D_wing(U.NC, U.NDW, U.NX, U.NT; verbose_level = U.verbose_print.level)
...
function randomGaugefields_2D_wing(
    NC,
    NX,
    NT,
    NDW;
...
@inline function Base.getindex(
    U::Shifted_Gaugefields_2D{NC,false},
    i1,
    i2,
    i3,
    i6,
...
@inline function Base.getindex(
    U::Shifted_Gaugefields_2D{NC,true},
    i1,
    i2,
    i3,
    i6,
...
function Base.getindex(
    ...
    i3,
    i6,
) where {T<:Shifted_Gaugefields_2D,μ}
    NT = u.parent.NT
    #NZ = u.parent.NZ
    #NY = u.parent.NY
    NX = u.parent.NX
...
function map_U!(
    U::Gaugefields_2D_wing{NC},
    f!::Function,
    V::Gaugefields_2D_wing{NC},
    iseven::Bool,
) where {NC}
    NT = U.NT
    #NZ = U.NZ
...
function construct_Λmatrix_forSTOUT!(
    Λ,
    δ_current::Gaugefields_2D_wing{NC},
    Q,
    u::Gaugefields_2D_wing{NC},
) where {NC}
    NT = u.NT
...
function exptU!(
    ...
    u::Gaugefields_2D_wing{NC},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_2D_wing,NC} #uout = exp(t*u)
...
function exptU!(
    uout::T,
    t::N,
    v::Gaugefields_2D_wing{3},
    temps::Array{T,1},
) where {N<:Number,T<:Gaugefields_2D_wing} #uout = exp(t*u)
    ww = temps[1]
    w = temps[2]
...
function Traceless_antihermitian!(
    vout::Gaugefields_2D_wing{NC},
    vin::Gaugefields_2D_wing{NC},
) where {NC}
    #NC = vout.NC
    fac1N = 1 / NC
    nv = vin.NV
        ...
        for ix = 1:NX
            for k2 = 1:NC
                for k1 = 1:NC
                    ...
                    @simd for k3 = 1:NC
                        @inbounds c[k1, k2, ix, it] +=
                            α * a[k1, k3, ix, it] * b[k3, k2, ix, it]
                    end
                end
            end
        end
...
function LinearAlgebra.mul!(
    ...
    for it = 1:NT
        ...
        #for iy=1:NY
        for ix = 1:NX
            @inbounds c[1, 1, ix, it] =
                β * c[1, 1, ix, it] + α * a[1, 1, ix, it] * b[1, 1, ix, it]
        end
        #end
        #end
    end
    ...
    if α != 0
        @inbounds for it = 1:NT
            ...
            @simd for ix = 1:NX
                ...
                b33 = b[3, 3, ix, it]
                c[1, 1, ix, it] =
                    (a11 * b11 + a12 * b21 + a13 * b31) * α + β * c[1, 1, ix, it]
                c[2, 1, ix, it] =
                    (a21 * b11 + a22 * b21 + a23 * b31) * α + β * c[2, 1, ix, it]
                c[3, 1, ix, it] =
                    (a31 * b11 + a32 * b21 + a33 * b31) * α + β * c[3, 1, ix, it]
                c[1, 2, ix, it] =
                    (a11 * b12 + a12 * b22 + a13 * b32) * α + β * c[1, 2, ix, it]
                c[2, 2, ix, it] =
                    (a21 * b12 + a22 * b22 + a23 * b32) * α + β * c[2, 2, ix, it]
                c[3, 2, ix, it] =
                    (a31 * b12 + a32 * b22 + a33 * b32) * α + β * c[3, 2, ix, it]
                c[1, 3, ix, it] =
                    (a11 * b13 + a12 * b23 + a13 * b33) * α + β * c[1, 3, ix, it]
                c[2, 3, ix, it] =
                    (a21 * b13 + a22 * b23 + a23 * b33) * α + β * c[2, 3, ix, it]
                c[3, 3, ix, it] =
                    (a31 * b13 + a32 * b23 + a33 * b33) * α + β * c[3, 3, ix, it]
