struct TA_Gaugefields_2D_mpi{NC,NumofBasis} <: TA_Gaugefields_2D{NC}
    ...
    function TA_Gaugefields_2D_mpi(u::Gaugefields_2D{NC}) where NC
        ...
        return new{NC,NumofBasis}(zeros(Float64,NumofBasis,u.PN[1],u.PN[2]),u.NX,u.NT,u.NC,NumofBasis,generators,
            u.PEs,u.PN,true,u.myrank,u.nprocs,u.myrank_xt    
        )
    end
end
...
function Base.getindex(x::T,i...) where T<: TA_Gaugefields_2D_mpi
    @inbounds return x.a[i...]
end


function barrier(x::TA_Gaugefields_2D_mpi)
    MPI.Barrier(comm)
end

function add_U!(c::TA_Gaugefields_2D_mpi{NC,NumofBasis},α::N,a::TA_Gaugefields_2D_mpi{NC,NumofBasis}) where {NC, N<:Number,NumofBasis}
    NT = c.NT
    #NZ = c.NZ
    #NY = c.NY
    NX = c.NX
...
function clear_U!(U::TA_Gaugefields_2D_mpi{NC,NumofBasis}) where {NC,NumofBasis}
...
function gauss_distribution!(p::TA_Gaugefields_2D_mpi{NC,NumofBasis};σ=1.0) where {NC,NumofBasis}
    d = Normal(0.0, σ)
    #NumofBasis = Uμ.NumofBasis
    pwork = rand(d,prod(p.PN)*NumofBasis)
    icount = 0
    @inbounds for it=1:p.PN[2]
        #for iz=1:p.PN[3]
            #for iy=1:p.PN[2]
...
function Base.:*(x::TA_Gaugefields_2D_mpi{NC,NumofBasis},y::TA_Gaugefields_2D_mpi{NC,NumofBasis}) where {NC,NumofBasis}
    #NumofBasis = Uμ.NumofBasis
    s = 0.0
    @inbounds for it=1:x.PN[2]
        #for iz=1:x.PN[3]
            #for iy=1:x.PN[2]
...
function Traceless_antihermitian_add!(c::TA_Gaugefields_2D_mpi{3,NumofBasis},factor,vin::Union{Gaugefields_2D_nowing_mpi{3},Gaugefields_2D_nowing_mpi{3}}) where NumofBasis
        ...
        #for iz=1:vin.PN[3]
            #for iy=1:vin.PN[2]
                @simd for ix=1:vin.PN[1]
                    ...
                    v22 = getvalue(vin,2,2,ix, it)
                    v33 = getvalue(vin,3,3,ix, it)
                    ...
                    v12 = getvalue(vin,1,2,ix, it)
                    v13 = getvalue(vin,1,3,ix, it)
                    v21 = getvalue(vin,2,1,ix, it)
                    v23 = getvalue(vin,2,3,ix, it)
                    v31 = getvalue(vin,3,1,ix, it)
                    v32 = getvalue(vin,3,2,ix, it)
...
function Traceless_antihermitian_add!(c::TA_Gaugefields_2D_mpi{2,NumofBasis},factor,vin::Union{Gaugefields_2D_nowing_mpi{2},Gaugefields_2D_nowing_mpi{2}}) where NumofBasis
    #error("Traceless_antihermitian! is not implemented in type $(typeof(vout)) ")
    fac12 = 1/2
    NX = vin.NX
    #NY = vin.NY
...
-----------------------------------------------------c
    """
function Traceless_antihermitian!(c::TA_Gaugefields_2D_mpi{3,NumofBasis},vin::Union{Gaugefields_2D_nowing_mpi{3},Gaugefields_2D_nowing_mpi{3}}) where NumofBasis
    #error("Traceless_antihermitian! is not implemented in type $(typeof(vout)) ")
    fac13 = 1/3
    NX = vin.NX
    #NY = vin.NY
...
function Traceless_antihermitian!(c::TA_Gaugefields_2D_mpi{NC,NumofBasis},vin::Union{Gaugefields_2D_nowing_mpi{NC},Gaugefields_2D_nowing_mpi{NC}}) where {NC,NumofBasis}
    @assert NC != 3 && NC != 2 
    #NC = vout.NC
    fac1N = 1/NC
    nv = vin.NV
        ...
        #for iz=1:vin.PN[3]
            #for iy=1:vin.PN[2]
                @simd for ix=1:vin.PN[1]
                    tri = 0.0
                    @simd for k=1:NC
                        tri += imag(getvalue(vin,k,k,ix, it))
                    end
                    tri *= fac1N
                    @simd for k=1:NC
                        #vout[k,k,ix, it] = (imag(getvalue(vin,k,k,ix, it))-tri)*im
                        matrix[k,k] = (imag(getvalue(vin,k,k,ix, it))-tri)*im
                    end

                    @simd for k2=k1+1:NC
                        vv = 0.5*(getvalue(vin,k1,k2,ix, it) - conj(getvalue(vin,k2,k1,ix, it)))
                        #vout[k1,k2,ix, it] = vv
...
function Traceless_antihermitian_add!(c::TA_Gaugefields_2D_mpi{NC,NumofBasis},factor,vin::Union{Gaugefields_2D_nowing_mpi{NC},Gaugefields_2D_nowing_mpi{NC}}) where {NC,NumofBasis}
...
function exptU!(uout::T,t::N,u::TA_Gaugefields_2D_mpi{NC,NumofBasis},temps::Array{T,1}) where {N <: Number, T <: Gaugefields_2D, NC,NumofBasis} #uout = exp(t*u)
...
function exptU!(uout::T,t::N,u::TA_Gaugefields_2D_mpi{3,NumofBasis},temps::Array{T,1}) where {N <: Number, T <: Gaugefields_2D,NumofBasis} #uout = exp(t*u)     
    ww = temps[1]
    w = temps[2]
    NT = u.NT
    #NZ = u.NZ
        ...
        #for iz=1:U.PN[3]
            #for iy=1:u.PN[2]
                for ix=1:u.PN[1]
                    ...
                    if csum == 0
                        v = 1
                        setvalue!(w,v,1,1,ix, it)# =   1 
                        v = 0
                        setvalue!(w,v,1,2,ix, it)
                        setvalue!(w,v,1,3,ix, it)# =    0 
                        setvalue!(w,v,2,1,ix, it)# =  0
                        v = 1
                        setvalue!(w,v,2,2,ix, it)# =   1 
                        v = 0
                        setvalue!(w,v,2,3,ix, it)# =   0
                        setvalue!(w,v,3,1,ix, it)# =   0 
                        setvalue!(w,v,3,2,ix, it)# =   0  
                        v = 1
                        setvalue!(w,v,3,3,ix, it)# =   1  
                
                        v = 1
                        setvalue!(ww,v,1,1,ix, it)# =   1
                        v = 0
                        setvalue!(ww,v,1,2,ix, it)# =   0
                        setvalue!(ww,v,1,3,ix, it)# =   0
                        setvalue!(ww,v,2,1,ix, it)# =   0
                        v = 1
                        setvalue!(ww,v,2,2,ix, it)# =   1 
                        v = 0
                        setvalue!(ww,v,2,3,ix, it)# =   0 
                        setvalue!(ww,v,3,1,ix, it)# =   0
                        setvalue!(ww,v,3,2,ix, it)# =   0  
                        v = 1
                        setvalue!(ww,v,3,3,ix, it)# =   1
                        continue
                    end
...
function exptU!(uout::T,t::N,u::TA_Gaugefields_2D_mpi{2,NumofBasis},temps::Array{T,1}) where {N <: Number, T <: Gaugefields_2D,NumofBasis} #uout = exp(t*u)     
    NT = u.NT
    #NZ = u.NZ
    #NY = u.NY
    NX = u.NX
        ...
        #for iz=1:U.PN[3]
            #for iy=1:u.PN[2]
                @simd for ix=1:u.PN[1]
                    ...
                    v = im*a1 + a2
                    setvalue!(uout,v,1,2,ix, it) 
                    #uout[2,1,ix, it) = 
                    v = im*a1 - a2
                    setvalue!(uout,v,2,1,ix, it) 
                    #uout[2,2,ix, it)=
                    v = cos(R) - im*a3
                    setvalue!(uout,v,2,2,ix, it) 
